...
const generatePremiumPDF = (rx, doctor) => {
  const patientId = rx.patient_phone ? `#PT-${rx.patient_phone.slice(-4)}` : "#PT-0000";
...
// ── Live Preview Panel ────────────────────────────────────────────────────────
function PreviewPanel({ form, doctor, draftRef }) {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const res = await fetch("/api/send-pharmacy", {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ pdfBase64, patientName: rx.patient_name }),
      });
  ...
  return (
    <div style={{
      ...
      <div style={{ padding: "14px 16px", display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
        ...
        }}>📤 {isSending ? "Sending..." : "Send to Pharmacy"}</button>
        ...
        {showSuccess && (
          ...
          >
            ...
            <motion.h3 initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.1 }}
              style={{ fontSize: 20, fontWeight: 800, color: C.text, margin: 0 }}>Sent to Pharmacy</motion.h3>
            <motion.p initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.2 }}
              style={{ fontSize: 14, color: C.muted, marginTop: 8 }}>Email delivered successfully.</motion.p>
          </motion.div>
        )}
      </AnimatePresence>
    </div>
  ...
  if (editRx !== prevEditRx) {
    ...
    if (editRx) {
      setForm({
        ...
        date:      editRx.date || new Date().toISOString().slice(0, 10),
        diagnosis: editRx.diagnosis || "",
        notes:     editRx.notes || "",
        follow_up: editRx.follow_up || "",
        status:    editRx.status || "active",
